...
func (l *LmodInstaller) GenerateInstallScript() string {
	...
	script.WriteString(fmt.Sprintf("export MODULEPATH=%s/Core\n", l.config.ModulePath))
	script.WriteString("\n")
...
func (l *LmodInstaller) GenerateSpackIntegrationScript() string {
	...
	script.WriteString("          # Exclude packages that are used for builds only\n")
	...
	script.WriteString("echo \"Linking Spack modules to system MODULEPATH...\"\n")
	script.WriteString(fmt.Sprintf("if [ -d \"%s\" ]; then\n", spackModulePath))
	script.WriteString(fmt.Sprintf("  ln -sf %s/* %s/Core/ 2>/dev/null || true\n", spackModulePath, l.config.ModulePath))
	script.WriteString("fi\n\n")
